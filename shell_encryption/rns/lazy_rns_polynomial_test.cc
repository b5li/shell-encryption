...
TYPED_TEST(LazyRnsPolynomialNegativeTest,
  ...
  ASSERT_OK_AND_ASSIGN(auto lazy, LazyRnsPolynomial<TypeParam>::CreateZero(
                                      log_n, this->moduli_));
  EXPECT_THAT(
      lazy.FusedMulSumAddInPlace(poly_reduced, poly, poly, this->moduli_),
      StatusIs(absl::StatusCode::kInvalidArgument,
               HasSubstr("must all be defined wrt `moduli`")));
  EXPECT_THAT(
      lazy.FusedMulSumAddInPlace(poly, poly_reduced, poly, this->moduli_),
      StatusIs(absl::StatusCode::kInvalidArgument,
               HasSubstr("must all be defined wrt `moduli`")));
  EXPECT_THAT(
      lazy.FusedMulSumAddInPlace(poly, poly, poly_reduced, this->moduli_),
      StatusIs(absl::StatusCode::kInvalidArgument,
               HasSubstr("must all be defined wrt `moduli`")));
}
